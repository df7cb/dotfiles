...
%IRSSI = (
    ...
    changed	=> 'Sun Nov 24 20:28:43 2002'
);
...
# friends.pl
my $friends_version = $VERSION . " (20021124)";
...
my $default_friends_allowed_flags = "aovikluLFdrm";
...
my $default_friends_file = "$ENV{HOME}/.irssi/friends";
...
sub is_allowed_flag {
	return 1 if (Irssi::settings_get_str('friends_allowed_flags') =~ /\Q$_[0]\E/);
	return 0;
}
...
sub del_friend($) {
	...
	foreach my $idx (@idxs) {
		...
		if (!(!defined $handle || grep(/^$handle$/i, @todelete))) {
			push(@todelete, $handle);
			del_host($idx, '*');
		}
	}
	for (my $idx = 0; $idx < @friends; $idx++) {
		if (grep(/^$friends[$idx]->{handle}$/i, @todelete)) {
...
sub event_modechange {
	...
	if (defined $channelInfo && ($nick ne $myNick)) {
		...
		if ($char eq "+") { $mode = "+";
			...
			if ($mode eq "+") {
				...
				if ($modeargs[$ptr] eq $myNick) {
					...
					$friendIdx = get_idx($nickInfo->{nick}, $nickInfo->{host}) if ($nickInfo);
					...
					if ($friendIdx == -1 || get_friends_flags($friendIdx, $noPrefix) eq "L") {
	...
	if ($gotOpped && defined $channelInfo) {
...
sub event_whois {
	...
	Irssi::printformat(MSGLEVEL_CRAP, 'friends_whois', get_handbyidx($idx), ($friends[$idx]->{globflags} ? $friends[$idx]->{globflags} : "none"));
}
...
Irssi::settings_add_str('misc', 'friends_allowed_flags', $default_friends_allowed_flags);
